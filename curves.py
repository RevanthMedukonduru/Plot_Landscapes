...
class CurveNet(Module):
    def __init__(self, num_classes, curve, architecture, num_bends, fix_start=True, fix_end=True,
        ...
        Visualization(self.net).structure_graph()
        
        self.curve_modules = []
        for delta_model in self.net.delta_models:
            for module in delta_model.modules():
                print("Modules are: ", module.__class__)
                if issubclass(module.__class__, CurveModule):
                    print("Curve Module Added: ", module.__class__)
                    self.curve_modules.append(module)

    def import_base_parameters(self, base_model, index, no_of_lora_particles):
        for i in range(no_of_lora_particles):
            base_parameters = list(base_model.delta_models[i].parameters())
            curve_parameters = list(self.net.delta_models[i].parameters())[index::self.num_bends]
            print(f"No.of B.Params: {len(base_parameters)} | C.Bef: {len(list(self.net.delta_models[i].parameters()))} | Aft: {len(curve_parameters)}")
            for curve_parameter, base_parameter in zip(curve_parameters, base_parameters):
                curve_parameter.data.copy_(base_parameter.data)
        ...
        # print("Len of base parameters: ", len(list(base_model.parameters())))
    ...
    def init_linear(self, no_of_lora_particles):
        for i in range(no_of_lora_particles):
            parameters = list(self.net.delta_models[i].parameters())
            for j in range(0, len(parameters), self.num_bends):
                weights = parameters[j:j+self.num_bends]
                for k in range(1, self.num_bends - 1):
                    alpha = k * 1.0 / (self.num_bends - 1)
                    weights[k].data.copy_(alpha * weights[-1].data + (1.0 - alpha) * weights[0].data)   
