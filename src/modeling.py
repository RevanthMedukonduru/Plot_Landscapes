...
class CurveImageEncoder(torch.nn.Module):
    def __init__(self, model, keep_lang=False):
        super().__init__()
    ...
    def forward(self, image, coeffs_t):
        assert self.model is not None
        return self.model.encode_image(image, coeffs_t)

    def __call__(self, inputs, coeffs_t):
        return self.forward(inputs, coeffs_t)
...
class CurveImageClassifier(torch.nn.Module):
    def __init__(self, image_encoder, classification_head):
        super().__init__()
        self.image_encoder = image_encoder
        self.classification_head = classification_head
    ...
    def forward(self, inputs, coeffs_t):
        features = self.image_encoder(inputs, coeffs_t)
        features = features.to(torch.float32)
        outputs = self.classification_head(features)
        return outputs
